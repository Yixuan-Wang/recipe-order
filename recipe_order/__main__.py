...
def train(
    flavor: Annotated[
        NetworkFlavor, typer.Argument(case_sensitive=False)
    ],
    ...
    match flavor:
        case NetworkFlavor.Perm:
            ...
            perm = Perm(PermOption(
                randomize_dataset=rand,
            ))

            perm.train(PermTrainOption(
        ...
        case NetworkFlavor.Pointer:
            ...
            pointer = Pointer(PointerOption(
                randomize_dataset=rand,
            ))

            pointer.train(PointerTrainOption(
                batch_size=8,
                learning_rate=5e-5,
