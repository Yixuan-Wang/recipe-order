...
def prune_matrix(adj: Tensor):
    adj = adj - adj.tril()
    _ = itertools.accumulate(itertools.repeat(adj), torch.matmul if adj.ndim == 2 else torch.bmm, initial=adj)
    ...
    *,
    ...
    for x, y in edges:
        if x >= graph_size or y >= graph_size:
            raise ValueError(f"Edge <{x}, {y}> goes beyond the graph size of {graph_size}.")
